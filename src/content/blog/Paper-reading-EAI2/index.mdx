...
这一篇是我非常喜欢的了。本身的话，虽然你看这个 pipeline 很像是某种双系统，而且论文中也可以看到类似频率差异的表述，但是实际上只是一个快慢系统罢了。其中 VLM 只是一个 VL encoder，而下面的部分则是 action encoder，最后放到一起 fusion。很有趣的一点是，这篇使用了
